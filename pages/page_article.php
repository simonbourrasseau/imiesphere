...
$getid = intval($_GET['article']);

$reqarticle = $bdd->prepare('SELECT * FROM article WHERE id_article = ?');
$reqarticle->execute(array($getid));
$articleinfo = $reqarticle->fetch();

$reqmembre = $bdd->prepare('SELECT * FROM espace_membre WHERE id_membre = ?');
$reqmembre->execute(array($articleinfo['id_membre']));
...
$membreinfo = $reqmembre->fetch();
 ...
 ?>
<h1>  <?php echo $articleinfo["titre"]; ?> </h1>
<p> <?php echo $articleinfo["introduction"]; ?> </p>
<p> <?php echo "Ecrit par : ".$membreinfo["prenom"]." ".$membreinfo["nom"]; ?> </p>
<p> <?php echo "<img id='article_image' src=".$articleinfo['image'].">" ?> </p>
 <?php echo "<p id='article_text'>".$articleinfo["corps_text"]."</p>"; ?>
      <?php echo "A Propos de l'auteur :"."<a href=".$membreinfo["linkedin"].">"."<img src='http://www.lesbonimenteurs.be/wp-content/uploads/2013/12/LinkedIn-Logo-02.png' width='7%'>"."</a>";
?>

</div>

	<!-- Pied de page-->
	<footer id="foot_page_article">
		<div id="pied_de_page">
			<center>
